...
По пути `public/env.js` можно настраивать различные параметры проекта

1. API_URL - Url на сервер проекта `trainer.online.server`.
...
4. Будет создана папка `dist`, в которой и будет находиться собранный дистрибутив;
5. Развернуть контейнер ``` docker compose up --build```;
